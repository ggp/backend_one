...
defmodule Seller do
  ...
  # the cond line permit to define if the data is ready to be send ...
  def get_stats_msg(%Seller{} = seller, %{seller_id: seller_id, time: time}) do
    ^seller_id = seller.id
    rtime = get_time_minutes_rounded(time)
    before_1_min = get_time_minutes_rounded(Timex.shift(time, minutes: -1))
    data = seller.data[rtime]
    after_1_min = get_time_minutes_rounded(Timex.shift(time, minutes: 1))
    data_before = seller.data[before_1_min]
    data_after = seller.data[after_1_min]
    cond do
      SellerData.is_filled?(data) && SellerData.is_enough?(data_after) ->
        {:send, create_stats_map(seller_id, data)}
      SellerData.is_filled?(data_before) && SellerData.is_enough?(data) ->
        {:send, create_stats_map(seller_id, data_before)}
      true ->
        {:wait, nil}
    end
  end
